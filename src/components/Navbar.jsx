    ...
    <div>
      ...
      >
        <div className="flex w-full flex-wrap items-center justify-between px-6">
          {/* Link kısmı navbar sol taraf */}
          <Link
            className="pr-2 text-2xl font-semibold flex justify-center "
            to="/"
          >
            <img src={logo} alt="logo" style={{ width: "7vh" }} />
            <span className="mx-3 ">React Movie App</span>
          </Link>

          {/* Navbar sag taraf  */}
          {/* Navbar Collapsible wrapper */}
          <div className="relative flex items-center">
            {currentUser && (
              <h5 className="mr-2 capitalize">{currentUser.displayName}</h5>
            )}
            <Switch />
            {/*   <Switch /> dark mode için componentimiz. */}
      ...
      {/*//! navbar fixed oldugundan akıştan çıktıve formun üst kısmı navbar altında kaldı bunu düzeltmek içinde navbar ınaltına boşbir div açıp ona bir height verdik ki bunu çözelim. */}
    </div>
  );
};
